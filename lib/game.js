...
var Game = function () {
    ...
    this.getStatus = function (player) {
        ...
        var result = {
            players: players.map(function (p) {
                return {name: p.name, pass: p.pass, points: p.getPoints(), total: p.getTotalPoints()};
            }),
    ...
    this.isValidForStepAboveOne = function (player, card) {
        if (this.isAvailCurrentSuit(player)) {
    ...
    this.isAvailCurrentSuit = function (player) {
        return this.hasCurrentSuit(this.playedCards[0].suit, player);
    };

    this.firstTrickValidator = function (player, card) {
        if (step > 1) {
            return this.isValidForStepAboveOne(player, card);
        }
        return card.suit == "club" && card.rank == "2";
    };

    this.firstStepAndHeart = function (card) {
        return step == 1 && card.suit == "heart";
    };

    this.isAbleToPlayHeart = function (player) {
        if (this.hasOnlyHeart(player)) {
            heartBroken = true;
        }
        return heartBroken;
    };

    this.isValidForStepAboveOneAndCurrentSuit = function (player) {
        return step > 1 && this.hasCurrentSuit(this.playedCards[0].suit, player);
    };

    this.isValidCard = function (player, card) {
        if (trick == 1) {
            return this.firstTrickValidator(player, card);
        }
        else if (this.firstStepAndHeart(card)) {
            return this.isAbleToPlayHeart(player);
        }
        else if (this.isValidForStepAboveOneAndCurrentSuit(player))
            return card.suit == this.playedCards[0].suit;
        else
            return true;
    };
    ...
    this.isGameOver = function () {
        return players.some(function (player) {
            return player.getTotalPoints() >= 100;
        });
    };
    ...
    this.getWinner = function () {
        ...
        var result = {
            winner: "Winner is : " + sortedPlayer[3].name,
        };
        return result;
    };
