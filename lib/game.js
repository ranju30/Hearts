...
var Game = function(){
	...
	this.getStatus = function(player){
		...
		if(players.length == 4){
			var playerName = players[(currentPlayerIndex+(round%4))%4].name;
		}
		var result = {
		...
		else if(!currentPlayer.pass && players.length == 4){
			result.instruction = 'Select 3 Cards and pass to ' +playerName;
		}
		else if(currentPlayer.pass && players.length == 4 && playerTurn == undefined){
			result.instruction = "Wait for others player to pass";
		}
		else{
	...
	this.getWinner = function(){
		...
		var result = {
			winner:'Game Over. Winner is : ' + sortedPlayer[3].name,
		};
		return result;
	};
